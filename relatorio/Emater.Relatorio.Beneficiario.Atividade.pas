unit Emater.Relatorio.Beneficiario.Atividade;
...
type
  TFrmRelatorioBeneficiarioAtividade = class(TFrmBaseRelatorio)
    ...
    DtStPrincipalATIVIDADE: TFIBStringField;
...
var
  FrmRelatorioBeneficiarioAtividade: TFrmRelatorioBeneficiarioAtividade;
...
procedure TFrmRelatorioBeneficiarioAtividade.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioBeneficiarioAtividade.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioBeneficiarioAtividade.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioBeneficiarioAtividade.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioBeneficiarioAtividade.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
