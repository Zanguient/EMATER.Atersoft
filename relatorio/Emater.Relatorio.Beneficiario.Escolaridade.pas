unit Emater.Relatorio.Beneficiario.Escolaridade;
...
type
  TFrmRelatorioBeneficiarioEscolaridade = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioBeneficiarioEscolaridade: TFrmRelatorioBeneficiarioEscolaridade;
...
procedure TFrmRelatorioBeneficiarioEscolaridade.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioBeneficiarioEscolaridade.BtnLimparClick(Sender: TObject);
begin
...
procedure TFrmRelatorioBeneficiarioEscolaridade.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioBeneficiarioEscolaridade.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioBeneficiarioEscolaridade.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
