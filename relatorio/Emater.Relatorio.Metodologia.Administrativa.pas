unit Emater.Relatorio.Metodologia.Administrativa;
...
type
  TFrmRelatorioMetodologiaAdministrativa = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioMetodologiaAdministrativa: TFrmRelatorioMetodologiaAdministrativa;
...
procedure TFrmRelatorioMetodologiaAdministrativa.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioMetodologiaAdministrativa.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioMetodologiaAdministrativa.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioMetodologiaAdministrativa.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioMetodologiaAdministrativa.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
