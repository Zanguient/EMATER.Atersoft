unit Emater.Relatorio.Metodologia.Funcionario;
...
type
  TFrmRelatorioMetodologiaFuncionario = class(TFrmBaseRelatorio)
    ...
    DtStGraficoTECNICO: TFIBStringField;
...
var
  FrmRelatorioMetodologiaFuncionario: TFrmRelatorioMetodologiaFuncionario;
...
procedure TFrmRelatorioMetodologiaFuncionario.BtnImprimirClick(Sender: TObject);
var
        ...
        begin
          ...
          if not VarIsNull(FrmFiltro.LkpCmbBxFuncionario.EditValue) then
            begin
              ...
              DtStGrafico.ParamByName('funcionario').AsInteger := FrmFiltro.LkpCmbBxFuncionario.EditValue;
            end
          else
            begin
              DtStPrincipal.ParamByName('funcionario').AsInteger := 0;
              DtStGrafico.ParamByName('funcionario').AsInteger := 0;
            end;
...
procedure TFrmRelatorioMetodologiaFuncionario.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioMetodologiaFuncionario.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioMetodologiaFuncionario.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioMetodologiaFuncionario.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
