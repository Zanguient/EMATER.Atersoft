unit Emater.Relatorio.Beneficiario.Faixa;
...
type
  TFrmRelatorioBeneficiarioFaixa = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioBeneficiarioFaixa: TFrmRelatorioBeneficiarioFaixa;
...
procedure TFrmRelatorioBeneficiarioFaixa.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioBeneficiarioFaixa.BtnLimparClick(Sender: TObject);
begin
...
procedure TFrmRelatorioBeneficiarioFaixa.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioBeneficiarioFaixa.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioBeneficiarioFaixa.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
