unit Emater.Relatorio.Fater.Atividade;
...
type
  TFrmRelatorioFaterAtividade = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioFaterAtividade: TFrmRelatorioFaterAtividade;
...
procedure TFrmRelatorioFaterAtividade.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioFaterAtividade.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioFaterAtividade.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioFaterAtividade.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioFaterAtividade.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
