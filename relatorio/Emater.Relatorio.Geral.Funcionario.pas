unit Emater.Relatorio.Geral.Funcionario;
...
type
  TFrmRelatorioGeralFuncionario = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioGeralFuncionario: TFrmRelatorioGeralFuncionario;
...
procedure TFrmRelatorioGeralFuncionario.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioGeralFuncionario.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioGeralFuncionario.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioGeralFuncionario.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioGeralFuncionario.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
