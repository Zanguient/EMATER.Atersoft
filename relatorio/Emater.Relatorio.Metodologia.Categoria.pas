unit Emater.Relatorio.Metodologia.Categoria;
...
type
  TFrmRelatorioMetodologiaCategoria = class(TFrmBaseRelatorio)
...
var
  FrmRelatorioMetodologiaCategoria: TFrmRelatorioMetodologiaCategoria;
...
procedure TFrmRelatorioMetodologiaCategoria.BtnImprimirClick(Sender: TObject);
begin
...
procedure TFrmRelatorioMetodologiaCategoria.BtnLimparClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmRelatorioMetodologiaCategoria.FormCreate(Sender: TObject);
begin
  inherited;
  FrmFiltro.EdtDataInicio.Date := EncodeDate(YearOf(Date), MonthOf(Date), 1);
  FrmFiltro.EdtDataFim.Date := EncodeDate(YearOf(Date), MonthOf(Date), DaysInMonth(Date));
...
procedure TFrmRelatorioMetodologiaCategoria.FormShow(Sender: TObject);
begin
  inherited;
  FrmFiltro.LkpCmbBxUnidade.EditValue := DtmSistemaModulo.UnidadeLocalID;
end;

procedure TFrmRelatorioMetodologiaCategoria.FrxPrincipalGetValue(const VarName: string; var Value: Variant);
var
